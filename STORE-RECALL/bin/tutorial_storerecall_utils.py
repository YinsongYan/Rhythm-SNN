"""
This utils code refers to the work: Bellec, G., Scherr, F., Subramoney, A., Hajek, E., Salaj, D., Legenstein, R., & Maass, W. (2020). A solution to the learning dilemma for recurrent networks of spiking neurons. Nature communications, 11(1), 3625.
...
def error_rate(z, num_Y, num_X, n_character):
    ...
    char_predict = torch.argmax(z, dim=2)
    char_true = num_Y
    char_input = num_X

    # error rate 1) Wrong characters
    char_correct = torch.eq(char_predict, char_true).float()
    character_errors = 1 - char_correct.mean()

    # error rate 2) wrong recalls
    recall_mask = torch.eq(char_input, n_recall_symbol)
    recalls_predict = char_predict[recall_mask]
    recalls_true = char_true[recall_mask]

    recall_correct = torch.eq(recalls_predict, recalls_true)
    recall_errors = (1 - recall_correct.float()).mean()

    # Get wrong samples
    sentence_id = torch.arange(shp[0]).unsqueeze(1).repeat(1, shp[1])
    recall_sentence_id = sentence_id[recall_mask]
    false_sentence_id_list = recall_sentence_id[~recall_correct]
