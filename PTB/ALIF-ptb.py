...
class NNModel(nn.Module):
    ...
    def snn_update(self, fc, inputs, mem, spike, tau_m, b, tau_adp, dt=1, isAdapt=1, b_j0 = 0.1):
        state = inputs.mm(fc)
    ...
    def snn_forward(self, input, hidden):
        ...
        buf = []
        for t in range(n_win):
            h1_mem, h1_spike, theta_h1, self.b_h1 = self.snn_update(self.snn_fc1, input[t], h1_mem, h1_spike, self.tau_m1, self.b_h1, self.tau_adp1)
            h2_mem, h2_spike, theta_h2, self.b_h2 = self.snn_update(self.snn_fc2, h1_spike, h2_mem, h2_spike, self.tau_m2, self.b_h2, self.tau_adp2)
            buf.append(h2_spike)
        stacked_output = torch.stack(buf, dim=0)
        # reshaped_tensor = torch.stack(buf, dim=0).view(n_win, batch_size, -1, self.n_res)
        # stacked_output = reshaped_tensor.mean(dim=-1)
...
if args.mode == "train":
    model_name = dataset_name + "_" + args.model + "_alif" + "_" + str(args.seed) + "_nhid" + str(args.nhid) + "_t" + str(args.bptt)
