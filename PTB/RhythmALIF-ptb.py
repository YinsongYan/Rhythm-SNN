...
class NNModel(nn.Module):
    def __init__(self, 
        ...
        if lstm_or_snn == "lstm":
            ...
            self.tau_adp1 = nn.Parameter(nn.init.normal_(torch.Tensor(nn_shape[0]), 200, 50))
            self.snn_fc2 = nn.Parameter(nn.init.kaiming_uniform_(torch.Tensor(nn_shape[0], nn_shape[1])))
            ...
            self.tau_adp2 = nn.Parameter(nn.init.normal_(torch.Tensor(nn_shape[0]), 200, 50))
            cycle_min = 10
            cycle_max = 50
            duty_cycle_min = 0.95
            duty_cycle_max = 0.99
    ...
    def Rhythmsnn_update(self, fc, inputs, mem, spike, mask, tau_m, b, tau_adp, dt=1, isAdapt=1, b_j0 = 0.1):
        ...
        mem = mem * (1 - reset_spike) * decay + state   # hard reset
        # mem = mem * self.decay - self.thresh * spike + state   # soft reset
        mem = torch.where(mask == 0, pre_mem, mem)
        now_spike = self.act_fun(mem - B) * mask
        return mem, now_spike.float(), B, b
    ...
    #     buf = []
    #     for t in range(n_win * n_res):
    #         h1_mem, h1_spike = self.Rhythmsnn_update(self.snn_fc1, input[t//n_res], h1_mem, h1_spike, self.mask1[:, t])
    #         h2_mem, h2_spike = self.Rhythmsnn_update(self.snn_fc2, h1_spike, h2_mem, h2_spike, self.mask2[:, t])
    #         buf.append(h2_spike)
    #     reshaped_tensor = torch.stack(buf, dim=0).view(n_win, n_res, batch_size, -1)
    #     stacked_output  = reshaped_tensor.mean(dim=1)
    #     return stacked_output, hidden

    def Rhythmsnn_forward(self, input, hidden):
        self.b_h1 = self.b_h2 = 0.1
        n_win, batch_size, input_size = input.size()
        h1_mem = h1_spike = torch.zeros(batch_size, self.nn_shape[0], device = self.device)
        h2_mem = h2_spike = torch.zeros(batch_size, self.nn_shape[1], device = self.device)
        buf = []
        for t in range(n_win):
            h1_mem, h1_spike, theta_h1, self.b_h1 = self.Rhythmsnn_update(self.snn_fc1, input[t], h1_mem, h1_spike, self.mask1[:, t], self.tau_m1, self.b_h1, self.tau_adp1)
            h2_mem, h2_spike, theta_h2, self.b_h2 = self.Rhythmsnn_update(self.snn_fc2, h1_spike, h2_mem, h2_spike, self.mask2[:, t], self.tau_m2, self.b_h2, self.tau_adp2)
            buf.append(h2_spike)
        stacked_output = torch.stack(buf, dim=0)
...
if args.mode == "train":
    model_name = dataset_name + "_" + args.model + "_alif" + "_" + str(args.seed) + "_" + str(args.nhid) + "_" + str(args.bptt)
