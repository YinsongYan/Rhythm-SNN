...
class NNModel(nn.Module):
    ...
    def Rhythmsnn_update(self, fc, inputs, mem, spike, mask, tau_m):
    ...
    def Rhythmsnn_forward(self, input, hidden):
        ...
        buf = []
        for t in range(n_win):
            h1_mem, h1_spike = self.Rhythmsnn_update(self.snn_fc1, input[t], h1_mem, h1_spike, self.mask1[:, t], self.tau_m1)
            h2_mem, h2_spike = self.Rhythmsnn_update(self.snn_fc2, h1_spike, h2_mem, h2_spike, self.mask2[:, t], self.tau_m2)
            buf.append(h2_spike)
        stacked_output = torch.stack(buf, dim=0)
...
if args.mode == "train":
    model_name = dataset_name + "_" + args.model + "_plif" + "_" + str(args.seed) + "_" + str(args.nhid) + "_" + str(args.bptt)
