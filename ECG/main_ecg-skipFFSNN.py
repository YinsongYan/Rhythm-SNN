...
from skip_FFSNN.spiking_ECG_model import FFSNN_mix
...
snn_ckp_dir = './exp/skip_FFSNN/checkpoint/'
snn_rec_dir = './exp/skip_FFSNN/record/'
...
train_record_path = 'Skip_FFSNN_mix_T{0}_decay{1}_thr{2}_lens{3}_arch{4}_skip{5}_lr{6}'\
    .format((1301), args.decay, args.thresh, args.lens,arch_name, skip_name, learning_rate) #args.save_path
train_chk_pnt_path = 'Skip_FFSNN_mix_T{0}_decay{1}_thr{2}_lens{3}_arch{4}_skip{5}_lr{6}.pt'\
    .format((1301), args.decay, args.thresh, args.lens, arch_name, skip_name, learning_rate) #args.chkp_path
...
net = FFSNN_mix(in_size = args.in_size, hidden_size=args.fc[0], output_size=args.out_size)
net = net.to(device)
print(net)
