...
from FFSNN.spiking_ECG_model import FFSNN
...
train_record_path = 'FFSNN_T{0}_decay{1}_thr{2}_lens{3}_arch{4}_lr{5}'\
    .format((1301), args.decay, args.thresh, args.lens,arch_name, learning_rate) #args.save_path
train_chk_pnt_path = 'FFSNN_T{0}_decay{1}_thr{2}_lens{3}_arch{4}_lr{5}.pt'\
    .format((1301), args.decay, args.thresh, args.lens, arch_name, learning_rate) #args.chkp_path
...
train_loader = DataLoader(train_data, shuffle=True, batch_size=batch_size, drop_last=False, num_workers=8)
test_data = TensorDataset(torch.from_numpy(test_x*1.),torch.from_numpy(test_y))
test_loader = DataLoader(test_data, shuffle=True, batch_size=batch_size, drop_last=False, num_workers=8)
############################################################

net = FFSNN(in_size = args.in_size, hidden_size=args.fc[0], output_size=args.out_size)
net = net.to(device)
print(net)
