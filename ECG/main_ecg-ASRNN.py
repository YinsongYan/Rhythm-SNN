...
from SRNN.spiking_ECG_model import SRNN_ALIF
...
train_record_path = 'SRNN_ALIF_step-wise_T{0}_lens{1}_arch{2}_lr{3}'\
    .format((1301), arch_name, args.lens, learning_rate)
train_chk_pnt_path = 'SRNN_ALIF_step-wise_T{0}_lens{1}_arch{2}_lr{3}.pt'\
    .format((1301), arch_name, args.lens, learning_rate)
...
net = SRNN_ALIF(in_size = args.in_size, hidden_size=args.fc[0], output_size=args.out_size)
net = net.to(device)
print(net)
