...
class SRNN_ALIF_mix_v3(nn.Module):
    ...
    def forward(self, input):
        ...
        for step in range(seq_num):
            ...
            hidden_mem, hidden_spike, theta_h, self.b_h = mem_update_adp_skip(h_input, hidden_mem, hidden_spike, self.tau_adp_h,
                                                                        self.tau_m_h, self.b_h, self.mask1[:, step], isAdapt=0)

            o_input = self.h2o(hidden_spike)
            output_mem, output_spike, theta_o, self.b_o = mem_update_adp_skip(o_input, output_mem, output_spike, self.tau_adp_o, 
                                                                         self.tau_m_o, self.b_o, self.mask2[:, step], isAdapt=1)
...
########################################################################
class SRNN_mix(nn.Module):
    """
    Mix skip_length within layers. With min and max skip value.
    """
    def __init__(self, in_size, hidden_size, output_size):
        super(SRNN_mix, self).__init__()
        self.input_size = in_size
        self.hidden_size = hidden_size
        self.output_size = output_size
    ...
    def forward(self, input):
        ...
        for step in range(seq_num):
            ...
            h2h_mem, h2h_spike = mem_update_hidden_skip_woDecay(h_input, h2h_mem, h2h_spike, self.mask1[:, step])
            h2o_mem, h2o_spike = mem_update_skip_woDecay(self.h2o, h2h_spike, h2o_mem, h2o_spike, self.mask2[:, step])
            mem_sum = h2o_mem 
