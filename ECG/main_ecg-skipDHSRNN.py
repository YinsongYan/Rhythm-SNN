...
from skipDHSRNN.spiking_ECG_model import rnn_test
...
snn_ckp_dir = './exp/skipDHSRNN/checkpoint/'
snn_rec_dir = './exp/skipDHSRNN/record/'
...
train_record_path = 'Skip_DHSNN_general_T{0}_decay{1}_thr{2}_lens{3}_arch{4}_pMax{5}_cMin{6}_cMax{7}_dcMin{8}_dcMax{9}_lr{10}_bran{11}_lown{12}_highn{13}'\
...
train_chk_pnt_path = 'Skip_DHSNN_general_T{0}_decay{1}_thr{2}_lens{3}_arch{4}_pMax{5}_cMin{6}_cMax{7}_dcMin{8}_dcMax{9}_lr{10}_bran{11}_lown{12}_highn{13}.pt'\
...
net = rnn_test(T=1301, low_n=args.low_n, high_n=args.high_n, branch= args.branch)
net = net.to(device)
print(net)
