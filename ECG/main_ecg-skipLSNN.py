...
from skip_LSNN.spiking_ECG_mode_LSNN import LSNN_mix
...
train_record_path = 'skip_LSNN_mix_T{0}_lens{1}_arch{2}_skip{3}_lr{4}'\
    .format((1301), arch_name, args.lens, skip_name, learning_rate)
train_chk_pnt_path = 'skip_LSNN_mix_T{0}_lens{1}_arch{2}_skip{3}_lr{4}.pt'\
    .format((1301), arch_name, args.lens, skip_name, learning_rate)
...
net = LSNN_mix(in_size = args.in_size, hidden_size=args.fc[0], output_size=args.out_size)
net = net.to(device)
print(net)
