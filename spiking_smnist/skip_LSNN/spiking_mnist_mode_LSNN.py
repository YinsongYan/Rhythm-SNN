...
class LSNN_multi_v2(nn.Module):
    ...
    def __init__(self, in_size=8):
        super(LSNN_multi_v2, self).__init__()
...
class LSNN_multi_mix_v2(nn.Module):
    """
    Similar method with paper "Accurate and efficient time-domain classification with adaptive spiking recurrent neural networks"
    Mix skip_length within layers
    """
    def __init__(self, in_size=8):
        super(LSNN_multi_mix_v2, self).__init__()
        self.input_size = in_size
        self.stride = input_size
        self.output_size = output_size
        self.T = 784 // self.stride
...
class LSNN_2RNN_mix_v3(nn.Module):
    """
    Similar method with paper "Accurate and efficient time-domain classification with adaptive spiking recurrent neural networks"
    Mix skip_length within layers. With min and max skip value.
    """
    def __init__(self, in_size=8):
        super(LSNN_2RNN_mix_v3, self).__init__()
        self.input_size = in_size
        self.stride = input_size
        self.output_size = output_size
        self.T = 784 // self.stride
...
class LSNN_multi_mix_v3_2RNN(nn.Module):
    """
    Similar method with paper "Accurate and efficient time-domain classification with adaptive spiking recurrent neural networks"
    Mix skip_length range within layers.  With min and max value.
    """
    def __init__(self, in_size=1):
        super(LSNN_multi_mix_v3_2RNN, self).__init__()
        self.input_size = in_size
        self.stride = input_size
        self.output_size = output_size
        self.T = 784 // self.stride
    ...
    def forward(self, input ):
        ...
        for step in range(self.T):
            ...
            ####################################################################
            h1_input = self.i2h_1(input_x) + self.h2h_1(h2h1_spike)
            h2h1_mem, h2h1_spike, theta_h1, self.b_h1 = self.spiking_neuron(h1_input,
                                                                          h2h1_mem, h2h1_spike,
                                                                          self.tau_adp_h1, self.b_h1, self.mask1[:, step])

            h2_input = self.i2h_2(h2h1_spike) + self.h2h_2(h2h2_spike)
            h2h2_mem, h2h2_spike, theta_h2, self.b_h2 = self.spiking_neuron(h2_input,
                                                                          h2h2_mem, h2h2_spike,
                                                                          self.tau_adp_h2, self.b_h2, self.mask2[:, step])

            h3_input = self.i2h_3(h2h2_spike) #+ self.h2h_3(h2h3_spike)
            h2h3_mem, h2h3_spike, theta_h3, self.b_h3 = self.spiking_neuron(h3_input,
                                                                          h2h3_mem, h2h3_spike,
                                                                          self.tau_adp_h3, self.b_h3, self.mask3[:, step])
