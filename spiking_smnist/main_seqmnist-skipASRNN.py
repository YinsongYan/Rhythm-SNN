...
from skip_SRNN.spiking_seqmnist_model import SRNN_ALIF_2RNN_general
...
snn_ckp_dir = './exp/skipASRNN/checkpoint/'
snn_rec_dir = './exp/skipASRNN/record/'
data_path = './data'
...
print('algo is %s, in_size = %d, lr = %.5f' %
      (args.algo, args.in_size, args.lr))
...
train_record_path = 'skipASRNN_general_in1_T{0}_lens{1}_arch{2}_pMax{3}_cMin{4}_cMax{5}_dcMin{6}_dcMax{7}_lr{8}'\
    .format(int(784/args.in_size), args.lens, arch_name, max_phase_name, min_cycle_name, max_cycle_name, min_dc_name, max_dc_name, learning_rate) #args.save_path
train_chk_pnt_path = 'skipASRNN_general_in1_T{0}_lens{1}_arch{2}_pMax{3}_cMin{4}_cMax{5}_dcMin{6}_dcMax{7}_lr{8}.pt'\
    .format(int(784/args.in_size), args.lens, arch_name, max_phase_name, min_cycle_name, max_cycle_name, min_dc_name, max_dc_name, learning_rate) #args.chkp_path
...
net = SRNN_ALIF_2RNN_general(in_size = 1)
net = net.to(device)
print(net)
