...
# data dir
train_data_root = "../data/SpeechCommands/speech_commands_v0.02"
test_data_root = "../data/SpeechCommands/speech_commands_v0.02"
...
n_fft = int(30e-3 * sr)  # 480
hop_length = int(10e-3 * sr)   # 160
# hop_length = int(16000 / 160)  # 100
n_mels = 40
fmax = 4000
fmin = 20
delta_order = 2
...
batch_size = 200
# generate dataset
train_dataset = SpeechCommandsDataset(train_data_root, label_dct, transform=transform, mode="train",
...
train_dataloader = DataLoader(train_dataset, batch_size=batch_size, num_workers=8, sampler=train_sampler,
                              collate_fn=collate_fn)

valid_dataset = SpeechCommandsDataset(train_data_root, label_dct, transform=transform, mode="valid",
                                      max_nb_per_class=None)
...
class Dense_test(nn.Module):
    def __init__(self, ):
        ...
        self.dense_1 = Rhy_spike_dense_test_denri_wotanh_R(40 * 3, n, tau_minitializer='uniform', low_m=0, high_m=0, tau_ninitializer = 'constant',low_n = 0,high_n = 2, vth=1, dt=1, branch=8,
                                                           cycle_min=10, cycle_max=50, duty_cycle_min=0.95, duty_cycle_max=1.0, phase_max=0.5, device=device, bias=is_bias)
        self.dense_2 = Rhy_spike_dense_test_denri_wotanh_R(n, n, tau_minitializer='uniform', low_m=0, high_m=0, tau_ninitializer = 'constant',low_n = 0,high_n = 2, vth=1, dt=1, branch=8,
                                                           cycle_min=10, cycle_max=50, duty_cycle_min=0.95, duty_cycle_max=1.0, phase_max=0.5, device=device, bias=is_bias)
        self.dense_3 = Rhy_spike_dense_test_denri_wotanh_R(n, n, tau_minitializer='uniform', low_m=0, high_m=0, tau_ninitializer = 'constant',low_n = 0,high_n = 2, vth=1, dt=1, branch=8,
                                                           cycle_min=10, cycle_max=50, duty_cycle_min=0.95, duty_cycle_max=1.0, phase_max=0.5, device=device, bias=is_bias)
    ...
    def forward(self, input):
        ...
        for i in range(seq_length):
            ...
            mem_layer1, spike_layer1 = self.dense_1.forward(input_x, i)

            mem_layer2, spike_layer2 = self.dense_2.forward(spike_layer1, i)

            mem_layer3, spike_layer3 = self.dense_3.forward(spike_layer2, i)
...
def test(data_loader, is_show=0):
    ...
    for i, (images, labels) in enumerate(data_loader):
        ...
        # images = images.view(-1, 3, 161, 40).to(device)
...
def train(epochs, criterion, optimizer, scheduler=None):
    acc_list = []
    best_acc = 0
    path = './checkpoint_101/dense_layer3_200neuron_branch8_initzero_MG_nl0h0_ml0h0_dcl0.95h1.0_cy10-50_pm0.5_seed0_'  # .pth'
...
scheduler = StepLR(optimizer, step_size=25, gamma=.5)  # 20  25
# epoch=0
epochs = 150
