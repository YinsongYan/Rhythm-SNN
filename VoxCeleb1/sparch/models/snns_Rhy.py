...
class SNN(nn.Module):
    ...
    def __init__(
        ...
        if neuron_type not in ["LIF", "PLIF", "adLIF", "RhyLIF", "RhyPLIF", "RhyadLIF"]:
...
class PLIFLayer(nn.Module):
    ...
    def __init__(
        ...
        self.alpha_log = nn.Parameter(torch.Tensor(self.hidden_size)).cuda()
        nn.init.constant_(self.alpha_log, - math.log(init_tau - 1))
        # self.alpha = nn.Parameter(torch.Tensor(self.hidden_size))
        # nn.init.uniform_(self.alpha, self.alpha_lim[0], self.alpha_lim[1])
    ...
    def _lif_cell(self, Wx):
        ...
        # Bound values of the neuron parameters to plausible ranges
        alpha = torch.clamp(self.alpha_log.sigmoid(), min=self.alpha_lim[0], max=self.alpha_lim[1])
...
class adLIFLayer(nn.Module):
    ...
    def __init__(
        ...
        self.bidirectional = bidirectional
        self.batch_size = self.batch_size * (1 + self.bidirectional)
        self.alpha_lim = [np.exp(-1 / 30), np.exp(-1 / 120)] # [-1/5, -1/25] [0.81, 0.96]
        self.beta_lim = [np.exp(-1 / 30), np.exp(-1 / 120)] # [-1/30, -1/120] [0.97, 0.99]
        self.a_lim = [-1.0, 1.0]
        self.b_lim = [0.0, 2.0]
        self.spike_fct = SpikeFunctionBoxcar.apply
...
class RhyLIFLayer(nn.Module):
    ...
    def create_general_mask(self, dim=128, c_min=4, c_max=8, min_dc=0.1, max_dc=0.9, phase_shift_max=0.5, T=10000):
        mask = []
    ...
    def _lif_cell(self, Wx):
        ...
        duty_cycle_max = 0.95
        phase_max = 0.3
        rhy_mask = self.create_general_mask(self.hidden_size, cycle_min, cycle_max, duty_cycle_min, duty_cycle_max, phase_max).to(device)
        ...
        for t in range(Wx.shape[1]):
            ...
            mask = rhy_mask[:, t].expand(Wx.shape[0], -1)

            # Compute membrane potential (LIF)
            pre_ut = ut
            ut = alpha * (ut - st) + (1 - alpha) * Wx[:, t, :]
            ut = torch.where(mask == 0, pre_ut, ut)

            # Compute spikes with surrogate gradient
            st = self.spike_fct(ut - self.threshold) * mask
            s.append(st)

        return torch.stack(s, dim=1)


class RhyPLIFLayer(nn.Module):
    """
    A single layer of Leaky Integrate-and-Fire neurons without layer-wise
    recurrent connections (LIF).
    ...
    def __init__(
        ...
        self.alpha_log = nn.Parameter(torch.Tensor(self.hidden_size))
        nn.init.constant_(self.alpha_log, - math.log(init_tau - 1))
        # self.alpha = nn.Parameter(torch.Tensor(self.hidden_size))
        # nn.init.uniform_(self.alpha, self.alpha_lim[0], self.alpha_lim[1])
    ...
    def _lif_cell(self, Wx):
        ...
        duty_cycle_max = 0.95
        phase_max = 0.5
        rhy_mask = self.create_general_mask(self.hidden_size, cycle_min, cycle_max, duty_cycle_min, duty_cycle_max,
                                            phase_max).to(device)
...
class RhyadLIFLayer(nn.Module):
    ...
    def __init__(
        ...
        self.bidirectional = bidirectional
        self.batch_size = self.batch_size * (1 + self.bidirectional)
        self.alpha_lim = [np.exp(-1 / 5), np.exp(-1 / 25)] # [-1/5, -1/25] [0.81, 0.96]
        self.beta_lim = [np.exp(-1 / 30), np.exp(-1 / 120)] # [-1/30, -1/120] [0.97, 0.99]
        self.a_lim = [-1.0, 1.0] # [-1.0, 1.0]
        self.b_lim = [0.0, 2.0] # [0.0, 2.0]
        self.spike_fct = SpikeFunctionBoxcar.apply
...
class RhyRadLIFLayer(nn.Module):
    """
