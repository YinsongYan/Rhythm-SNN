...
class MaskedLinear(nn.Module):
    def __init__(self, in_features, out_features, bias=True):
        ...
        nn.init.orthogonal_(self.linear.weight)#keep the diagnal to be zero

        # Create a mask with ones on the diagonal
...
class ElementWiseRecurrentContainer(MemoryModule):
    def __init__(self, sub_module: nn.Module, element_wise_function=lambda x, y: x + y, step_mode='s', hid_dim=64):
        ...
        # self.hid_weight = MaskedLinear(hid_dim, hid_dim, bias=False)#masked recurrent
