...
class ALIF(nn.Module):
    ...
    def forward(self, input, task='duration'):
        ...
        for i in range(input_steps):
            ...
            d2o_mem = self.act(self.d2o(r2_spike))
            out.append((d2o_mem))
...
class RhythmALIF(nn.Module):
    ...
    def forward(self, input, task='duration'):
        ...
        for i in range(input_steps):
            input_x = input[:, :, i]
            h_input = self.i2h(input_x.float()) + self.h2h(r1_spike)
            r1_mem, r1_spike, theta_r1, self.b_r1 = mem_update_adp_skip(h_input, r1_mem, r1_spike, self.tau_adp_r1, self.b_r1, self.tau_m_r1, self.mask1[:, i])


            d_input = self.h2d(r1_spike)
            # r2_mem, r2_spike, theta_r2, self.b_r2 = mem_update_adp(d_input, r2_mem, r2_spike, self.tau_adp_r2,
            #                                                        self.tau_m_r2, self.b_r2)
            r2_mem, r2_spike, theta_r2, self.b_r2 = mem_update_adp_skip(d_input, r2_mem, r2_spike, self.tau_adp_r2, self.b_r2, self.tau_m_r2, self.mask2[:, i])
...
class DEXAT(nn.Module):
    def __init__(self, input_size=2, hidden_dims=[64, 64], output_size=1, time_window=1):
        ...
        self.tau_adp_r11 = nn.Parameter(torch.Tensor(self.r1_dim))
        self.tau_adp_r12 = nn.Parameter(torch.Tensor(self.r1_dim))
        self.tau_adp_r21 = nn.Parameter(torch.Tensor(self.r2_dim))
        self.tau_adp_r22 = nn.Parameter(torch.Tensor(self.r2_dim))
        ...
        nn.init.normal_(self.tau_adp_r11, 30, 0)
        nn.init.normal_(self.tau_adp_r12, 700, 0)
        nn.init.normal_(self.tau_adp_r21, 30, 0)
        nn.init.normal_(self.tau_adp_r22, 700, 0)
    ...
    def forward(self, input, task='duration'):
        ...
        for i in range(input_steps):
            input_x = input[:, :, i]
            h_input = self.i2h(input_x.float()) + self.h2h(r1_spike)
            r1_mem, r1_spike, theta_r1, b_r11, b_r12 = mem_update_adp_DEXAT(h_input, r1_mem, r1_spike, self.tau_adp_r11, self.tau_adp_r12, self.tau_m_r1, b_r11, b_r12)

            d_input = self.h2d(r1_spike)
            r2_mem, r2_spike, theta_r2, b_r21, b_r22 = mem_update_adp_DEXAT(d_input, r2_mem, r2_spike, self.tau_adp_r21, self.tau_adp_r22, self.tau_m_r2, b_r21, b_r22)
...
class RhythmDEXAT(nn.Module):
    def __init__(self, input_size=2, hidden_dims=[64, 64], output_size=1, time_window=1):
        ...
        nn.init.normal_(self.tau_adp_r11, 30, 10)
        nn.init.normal_(self.tau_adp_r12, 700, 10)
        nn.init.normal_(self.tau_adp_r21, 30, 10)
        nn.init.normal_(self.tau_adp_r22, 700, 10)
    ...
    def forward(self, input, task='duration'):
        ...
        for i in range(input_steps):
            input_x = input[:, :, i]
            h_input = self.i2h(input_x.float()) + self.h2h(r1_spike)
            r1_mem, r1_spike, theta_r1, b_r11, b_r12 = mem_update_adp_DEXAT_skip(h_input, r1_mem, r1_spike, self.tau_adp_r11,
                                                                            self.tau_adp_r12, self.tau_m_r1, b_r11,
                                                                            b_r12, self.mask1[:, i])

            d_input = self.h2d(r1_spike)
            r2_mem, r2_spike, theta_r2, b_r21, b_r22 = mem_update_adp_DEXAT_skip(d_input, r2_mem, r2_spike, self.tau_adp_r21,
                                                                            self.tau_adp_r22, self.tau_m_r2, b_r21,
                                                                            b_r22, self.mask2[:, i])
    ...
    def get_grads(self, input):
        ...
        for i in range(input_steps):
            input_x = input[:, :, i]
            h_input = self.i2h(input_x.float()) + self.h2h(r1_spike)
            r1_mem, r1_spike, theta_r1, b_r11, b_r12 = mem_update_adp_DEXAT_skip(h_input, r1_mem, r1_spike,
                                                                                 self.tau_adp_r11,
                                                                                 self.tau_adp_r12, self.tau_m_r1, b_r11,
                                                                                 b_r12, self.mask1[:, i])

            d_input = self.h2d(r1_spike)
            r2_mem, r2_spike, theta_r2, b_r21, b_r22 = mem_update_adp_DEXAT_skip(d_input, r2_mem, r2_spike,
                                                                                 self.tau_adp_r21,
                                                                                 self.tau_adp_r22, self.tau_m_r2, b_r21,
                                                                                 b_r22, self.mask2[:, i])
