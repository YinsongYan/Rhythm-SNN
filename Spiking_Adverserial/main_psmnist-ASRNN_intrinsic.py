...
from ASRNN.spiking_psmnist_model import SRNN_ALIF_2RNN, ASRNN_thermal, ASRNN_silence, W_Quantize  # ASRNN_mismatch
...
# Noise model
# net_mm = ASRNN_mismatch(in_size=1, std_p=level_factor)
net_thermal = ASRNN_thermal(in_size=1, noise_std=level_factor)
net_silence = ASRNN_silence(in_size=1, silence_p=level_factor)

# Load pre-trained models
state = torch.load(snn_ckp_dir + pretrained_model)
net.load_state_dict(state['model_state_dict'])#, strict=False)
# net_mm.load_state_dict(state['model_state_dict'])#, strict=False)
net_thermal.load_state_dict(state['model_state_dict'])#, strict=False)
net_silence.load_state_dict(state['model_state_dict'])#, strict=False)
...
def plot_images(original, perturbed, original_hidden_layers, perturbed_hidden_layers, noise_type):
    ...
    plt.show()
    plt.savefig('./plot/figures_intrinsic/{}_ASRNN_hidden_representaion.pdf'.format(noise_type),   bbox_inches='tight')
...
def plot_diff_images(original, perturbed, original_hidden_layers, perturbed_hidden_layers, noise_type):
    ...
    plt.savefig('./plot/figures_intrinsic/{}_ASRNN_hid_feats_diff.pdf'.format(noise_type),   bbox_inches='tight')
    plt.show()
