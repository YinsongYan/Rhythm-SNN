...
def mem_update_adp_skip(inputs, mem, spike, tau_adp, tau_m, b, mask, dt=1, isAdapt=1):
...
class RNN_custom(nn.Module):
    def __init__(self, input_size, stride, hidden_dims, output_size, DC_f='adp-spike'):
    ...
    def forward(self, input):
        ...
        batch_size, seq_num = input.shape
        ...
        for i in range(input_steps):
            ...
            d1_mem, d1_spike, theta_d1, self.b_d1 = mem_update_adp(self.dense1(r2_spike), d1_mem, d1_spike,
                                                                   self.tau_adp_d1, self.tau_m_d1, self.b_d1)

            if self.DC_f[:3] == 'adp':
                d2o_mem, d2o_spike, theta_o, self.b_o = mem_update_adp(self.d2o(d1_spike), d2o_mem, d2o_spike,
                                                                       self.tau_adp_o, self.tau_m_o, self.b_o)
...
class RNN_custom_keras(nn.Module):
    def __init__(self, input_size, stride, hidden_dims, output_size, DC_f='adp-spike'):
        super(RNN_custom_keras, self).__init__()
        self.DC_f = DC_f
        self.stride = stride
    ...
    def forward(self, input):
        N = input.size(0)
        mask1 = self.create_mask(self.r1_dim, skip_length[0])
        mask2 = self.create_mask(self.r2_dim, skip_length[1])
        mask3 = self.create_mask(self.d1_dim, skip_length[2])
        ...
        for i in range(input_steps):
            ...
            # print(input_x.shape)
            h_input = self.i2h(input_x.float()) + self.h2h(r1_spike)
            r1_mem, r1_spike, theta_r1, self.b_r1 = mem_update_adp_skip(h_input, r1_mem, r1_spike, self.tau_adp_r1,
                                                                   self.tau_m_r1, self.b_r1, mask1[:, i])

            d_input = self.h2d(r1_spike) + self.d2d(r2_spike)
            r2_mem, r2_spike, theta_r2, self.b_r2 = mem_update_adp_skip(d_input, r2_mem, r2_spike, self.tau_adp_r2,
                                                                   self.tau_m_r2, self.b_r2, mask2[:, i])

            d1_mem, d1_spike, theta_d1, self.b_d1 = mem_update_adp_skip(self.dense1(r2_spike), d1_mem, d1_spike,
                                                                   self.tau_adp_d1, self.tau_m_d1, self.b_d1, mask3[:, i])
