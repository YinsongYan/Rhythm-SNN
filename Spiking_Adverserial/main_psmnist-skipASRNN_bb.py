...
# from skip_FFSNN.Hyperparameters_psmnist import args
...
bb_snn_ckp_dir = os.path.join('./exp/ASRNN/checkpoint/')
bb_snn_rec_dir = os.path.join('./exp/ASRNN/record/')
...
bb_model = 'SRNN_ALIF__in4_T784.0_lens0.2_arch64-256-256_lr0.01_tau5.pt'
...
# Config bb model
bb_net = SRNN_ALIF_2RNN(in_size=1)
...
def plot_images(original, perturbed, original_hidden_layers, perturbed_hidden_layers, noise_type):
    ...
    plt.show()
    plt.savefig('./plot/figures_attack/{}_skipASRNN_bb_hidden_representaion.pdf'.format(noise_type), bbox_inches='tight')
...
def plot_diff_images(original, perturbed, original_hidden_layers, perturbed_hidden_layers, noise_type):
    ...
    plt.savefig('./plot/figures_attack/{}_skipASRNN_bb_hid_feats_diff.pdf'.format(noise_type), bbox_inches='tight')
    plt.show()
