...
def perturbation_distance(original_image, adversarial_image, p=2):
    """
    ...
    if p == float('inf'):
        # L_inf norm
        return difference.abs().max()
    else:
        # L_p norm
        return torch.norm(difference.view(difference.size(0), -1), p=p, dim=1).mean()
