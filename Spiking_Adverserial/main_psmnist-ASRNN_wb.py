...
snn_ckp_dir = os.path.join('./exp/ASRNN/checkpoint/')
snn_rec_dir = os.path.join('./exp/ASRNN/record/')
data_path = './data'
...
atk_fgsm = attack.FGSM(net, eps=eps_factor / 255)

atk_pgd = attack.PGD(net, eps=eps_factor / 255, alpha=0.01, steps=7)

atk_gn = attack.GN(net, eps=eps_factor / 255)

atk_spn = attack.SPN(net, eps=noise_p)

atk_ipn = attack.IPN(net, eps=noise_p)
...
def plot_images(original, perturbed, original_hidden_layers, perturbed_hidden_layers, noise_type):
    ...
    plt.show()
    plt.savefig('./plot/figures_attack/{}_ASRNN_wb_hidden_representaion.pdf'.format(noise_type),  bbox_inches='tight')
...
def plot_diff_images(original, perturbed, original_hidden_layers, perturbed_hidden_layers, noise_type):
    ...
    for i, (orig_hidden, pert_hidden) in enumerate(zip(original_hidden_layers, perturbed_hidden_layers)):
        ...
        #im_diff = axes[1, i + 1].imshow(diff_hidden_reshaped, cmap='viridis', vmin=-0.7, vmax=0.8)
        im_diff = axes[1, i + 1].imshow(diff_hidden_reshaped, cmap='viridis', vmin=-0.7, vmax=0.8) # For GN
    ...
    plt.savefig('./plot/figures_attack/{}_ASRNN_wb_hid_feats_diff.pdf'.format(noise_type),  bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    for epoch in range(start_epoch, start_epoch + num_epochs):
        ...
        # acc_gn = val_analysis(net, testloader, device, atk_gn)
        # print('Analysing SPN.....')
        # acc_spn = val_analysis(net, testloader, device, atk_spn)
        # print('Analysing IPN.....')
        # acc_ipn = val_analysis(net, testloader, device, atk_ipn)
        # print('acc_test:', acc_test)
