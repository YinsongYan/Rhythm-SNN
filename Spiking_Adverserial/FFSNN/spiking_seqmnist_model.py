...
'''
decay_cfg = [torch.rand(cfg_fc[0], device=device),
         torch.rand(cfg_fc[1], device=device),
         torch.rand(cfg_fc[2], device=device)]
'''
...
class FFSNN_v2_fr(nn.Module):
    """
    Similar method with paper "Accurate and efficient time-domain classification with adaptive spiking recurrent neural networks"
    """
    def __init__(self, in_size=8):
        super(FFSNN_v2_fr, self).__init__()
    ...
    def forward(self, input):
        ...
        for step in range(time_window):   # input [N, 28, T]
            ...
            #### Calculate spike count ####
            h1_nonzeros += torch.count_nonzero(h1_spike)
            h2_nonzeros += torch.count_nonzero(h2_spike)
            h3_nonzeros += torch.count_nonzero(h3_spike)
            h1_neurons += torch.numel(h1_spike)
            h2_neurons += torch.numel(h2_spike)
            h3_neurons += torch.numel(h3_spike)

            h1_fr_list.append(h1_nonzeros / h1_neurons)
            h2_fr_list.append(h2_nonzeros / h2_neurons)
            h3_fr_list.append(h3_nonzeros / h3_neurons)

        outputs = output_sum / time_window
        hidden_spk = [h1_spike_sums/time_window, h2_spike_sums/time_window, h3_spike_sums/time_window]
        h1_fr_list = torch.tensor(h1_fr_list)
        h2_fr_list = torch.tensor(h2_fr_list)
        h3_fr_list = torch.tensor(h3_fr_list)
        layer_fr= [h1_fr_list, h2_fr_list, h3_fr_list]
        return outputs, hidden_spk, layer_fr #n_nonzeros/n_neurons
...
class FFSNN_v2_bb(nn.Module):
    ...
    def __init__(self, in_size=8, bias=True):
        ...
        self.mask1 = self.create_mask(cfg_fc[0], 4)
        self.mask2 = self.create_mask(cfg_fc[1], 5)
        self.mask3 = self.create_mask(cfg_fc[2], 6)
