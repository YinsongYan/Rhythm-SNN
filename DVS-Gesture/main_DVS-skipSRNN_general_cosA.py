...
from skipSRNN.spiking_dvs_model import skipSRNN_general
from skipSRNN.Hyperparameters import args
...
snn_ckp_dir = './logs/skipSRNN/checkpoint/'
snn_rec_dir = './logs/skipSRNN/record/'
data_path = './data'
...
print('algo is %s, thresh = %.2f, lens = %.2f, decay = %.2f, in_size = %d, lr = %.5f, T = %d' %
      (args.algo, args.thresh, args.lens, args.decay, args.in_size, args.lr, seq_len))
...
train_record_path = 'SkipSRNN_general_T{0}_decay{1}_thr{2}_lens{3}_arch{4}_pMax{5}_cMin{6}_cMax{7}_dcMin{8}_dcMax{9}_lr{10}_s{11}'\
...
train_chk_pnt_path = 'SkipSRNN_general_T{0}_decay{1}_thr{2}_lens{3}_arch{4}_pMax{5}_cMin{6}_cMax{7}_dcMin{8}_dcMax{9}_lr{10}_s{11}.pt'\
